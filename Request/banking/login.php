...
$nduckien = $conn->prepare("SELECT userlogin FROM adminpanel WHERE userlogin = ?");
$nduckien->execute([$userloginmbbank]);
$existingAccount = $nduckien->fetch();

if ($existingAccount) {
    // Update thông tin tài khoản nếu đã tồn tại
    $nduckien = $conn->prepare("UPDATE adminpanel SET stk = ?, name = ?, password = ?, sessionId = ?, deviceId = ?, token = ?, time = ? WHERE userlogin = ?");
    ...
    $nduckien = $conn->prepare("INSERT INTO adminpanel (userlogin, stk, name, password, sessionId, deviceId, token, time)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
