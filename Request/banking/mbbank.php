...
function layDuLieuNguoiDungTuToken($_token, $conn)
{
    ...
    $nduckien = $conn->prepare($sql);
    $nduckien->execute([$_token]);
    return $nduckien->fetch(PDO::FETCH_ASSOC);
}
...
function capNhatDuLieuNguoiDung($duLieuNguoiDung, $MB, $conn)
{
    ...
    if ($phienDangNhap['result']['message'] == "Capcha code is invalid") {
        ...
        $nduckien = $conn->prepare($sql);
        $nduckien->execute([$phienDangNhap['cust']['nm'], $duLieuNguoiDung['password'], $phienDangNhap['sessionId'], $MB->deviceIdCommon_goc, time(), $duLieuNguoiDung['userlogin']]);
    }
}
...
function xuLyGiaoDich($giaoDich, $conn)
{
    ...
    $tenNganHang = $giaoDich['bankName'] ?: "Ngọc Rồng Light";
        ...
        <div>
            ...
            <ul class="pagination">
                ...
                <?php for ($page = 1; $page <= $totalPages; $page++): ?>
                    <li>
                        <a href="?key=nduckien&page=<?= $page ?>" <?= ($page == $current_page) ? 'class="active"' : '' ?>>
                            <?= $page ?>
                        </a>
                    </li>
                <?php endfor; ?>
